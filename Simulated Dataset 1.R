...
for (error_type in error_types) {
  ...
  for (i in 1:50) {
    ...
    for(name in names(distributions)) {
      ...
      average_beta <- eigenfunctions_eval[,1:5] %*% colMeans(res$beta[,2:6])
      MSE_beta <- sum((true_beta - average_beta)^2) / length(true_beta)
      MSE_accum <- accumulate_scores(MSE_accum, name, MSE_beta)
      MSE_alpha1 <- sum((true_alpha - res$beta[,1])^2) / length(res$beta[,1])
      ...
      bias_alpha1 <- sum(true_alpha - res$beta[,1]) / length(res$beta[,1])
      bias_accum_a1 <- accumulate_scores(bias_accum_a1, name, bias_alpha1)
    }
  }
  
  # Average the accumulated results
  avg_scores <- lapply(list(dic_scores_accum, eaic_scores_accum, ebic_scores_accum, LPML_scores_accum, waic1_scores_accum, MSE_accum, bias_accum, MSE_accum_a1, bias_accum_a1), function(x) x / 50)
  ...
  final_results_b <- add_to_results_b(final_results_b, error_type, rounded_scores_b)
}
...
simulated_data <- simulate_functional_data(sample_size, num_obs, num_bases, time_grid, "Normal")
basis_expansion <- simulated_data$basis_expansion
fpca_results <- perform_fpca(basis_expansion, time_grid, num_basis = 170, num_scores = 5)
...
ggplot(explained_data, aes(x = PC, y = Cum_sum)) +
  ...
  annotate("text", x = var_80, y = 0.9, label = paste("FPC", var_80), vjust = -2, hjust = -0.5) +
